...
4. `govm.XXX` means that the value of the `govm.cfg` is used in this context.
5. `GOVM-ID` is referring to the `hex-id` that is generated by `govm` for the virtual-machine for future unique references.
6. `MOUNTING_POINT: <default-value>-[optional|required]` 
