...
# Documentation
The following sections will explain in detail how to use `govm`. Before you start reading the documentation some conventions:
...
2. `-someflag [options]` options is always referring to the options explained in [Usage](#usage) section.
3. `$VARIABLE` is meaning that this variables can be set by you or is an intern variable that is getting used for example `$GOVM-ID` is referring to the `hex-id` that is generated by `govm` for the virtual-machine for future unique references.  (obviously $HOME, $PATH etc... have still the same linux meaning) 
4. `govm.XXX` means that the value of the `govm.cfg` is used.
5. `<config-option>: <default-value> -[opt|req]`. Using this syntax is giving you the oppurtunity to see in a one liner which defaults the config-options in the different .cfg have and if they are **required(req)** or **optional(opt)**
...
Variables yhat you want to access 
...
use them in yout vagrantfile but the validation is then satisfied. <br/>
